...
def extract_patches(X, stride=1, w_size=3):
    N = X.shape[0]
    K = (X.shape[1]-w_size)/stride + 1
    X_patched = np.zeros((N, K, K, w_size*w_size*3), dtype=float)
    # do whitening here
    for idx in range(N):
        for i in range(K):
            for j in range(K):
                X_patched[idx,i,j,:] = np.reshape(X[idx,i*w_size:(i+1)*w_size,j*w_size:(j+1)*w_size,:],(w_size*w_size*3,))
